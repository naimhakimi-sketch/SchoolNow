...
class StudentManagementService {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> streamApprovedStudents(String driverId) {
    return _studentsRef(driverId).orderBy('created_at', descending: true).snapshots();
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> streamPendingRequests(String driverId) {
    // Avoid composite index requirement (status + created_at).
    // We'll sort client-side in the UI.
    return _requestsRef(driverId).where('status', isEqualTo: 'pending').snapshots();
  }
  ...
  Future<void> approveRequest({
    ...
    await _db.runTransaction((tx) async {
      ...
      if (parentId.isNotEmpty) {
        final childRef = parents.doc(parentId).collection('children').doc(studentId);
