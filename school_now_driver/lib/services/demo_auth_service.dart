...
class DemoAuthService {
  ...
  static Future<void> setupDemoUser() async {
    ...
    final students = db.collection('drivers').doc(demoUid).collection('students');
    await students.doc('student_demo_001').set(
      {
        'student_name': 'Ali Demo',
        'parent_name': 'Parent Ali',
        'parent_phone': '+60111111111',
        'pickup_location': {
          'lat': 3.1520,
          'lng': 101.6860,
    ...
    final requests = db.collection('drivers').doc(demoUid).collection('service_requests');
