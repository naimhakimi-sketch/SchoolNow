...
class _HomeAddressPickerPageState extends State<HomeAddressPickerPage> {
  ...
  void _onTap(LatLng pos) {
    setState(() {
      _selected = pos;
      // Tapping refines the pin position; keep last known display name if present.
    });
  }
  ...
  Future<void> _searchAddress() async {
    ...
    try {
      ...
      final resp = await http.get(
        uri,
        headers: const {
          'User-Agent': 'school_now_driver/1.0 (home address picker)',
        },
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Select Home Address'),
        actions: [
          TextButton(
            onPressed: _confirm,
